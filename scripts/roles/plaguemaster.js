module.exports = {
   chooseAction: function(guild,game,player) {
       if(player.isAlive){
           let mojiArray = [];
           let collector;
           guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"Select a player to infect, infected players will die the next night."+"```").catch(console.error).then(msgg => {
               game.players.forEach(playerr => {
                ...
                collector.on('collect', (reaction, user) => {
                    ...
                    game.players.forEach(playerr => {
                        if(reaction.emoji.name === playerr.emoteName){
                            ...
                            console.log(player.displayName+" has selected "+playerr.displayName+" as the Plague Master.");
                        }
                ...
                collector.on('end', collected => {
                    if(variableName != null){
                        guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected "+variableName+" to infect."+"```").catch(console.error);
                    }
                    else{
