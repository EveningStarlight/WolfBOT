module.exports = {
   chooseAction: function(guild,game,player) {
       if(player.isAlive){
           let mojiArray = [];
           let collector;
           guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"Select a player to cover in water, or select the snow to freeze."+"```").catch(console.error).then(msgg => {
                ...
                collector.on('collect', (reaction, user) => {
                    ...
                    game.players.forEach(playerr => {
                        if(reaction.emoji.name === playerr.emoteName){
                            ...
                            console.log(player.displayName+" has selected "+playerr.displayName+" as the Cryogenicist.");
                        }
                        else if(reaction.emoji.name === '❄️'){
                            variableName = "Freeze";
                            player.action = "Freeze";
                            game.updateActions(user);
                            console.log(player.displayName+" has selected Freeze as the Cryogenicist.");
                        }
                ...
                collector.on('end', collected => {
                    if(variableName != null){
                        if(variableName == "Freeze"){
                            guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected to freeze everyone covered in water."+"```").catch(console.error);
                            
                        }
                        else{
                            guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected "+variableName+" to cover in water."+"```").catch(console.error);
                        }
