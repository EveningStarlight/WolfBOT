module.exports = {
   chooseAction: function(guild,game,player) {
       if(player.isAlive){
               ...
               game.players.forEach(playerr => {
                    ...
                    game.players.forEach(playerr => {
                        if(reaction.emoji.name === playerr.emoteName){
                            ...
                            if(selectedOne == selectedTwo){
                                ...
                                game.updateActions(user);
                                console.log(player.displayName+" has selected "+selectedOne+" and "+selectedTwo+" as the Chronomancer.");
                            }
                            else{
                ...
                collector.on('end', collected => {
                    if(selectedOne != null && selectedTwo != null){
                        guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected "+selectedOne+" to protect and "+selectedTwo+" to kill in their place."+"```").catch(console.error);
                    }
                    else{
           ...
           guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"Select a player to kill in their place."+"```").catch(console.error).then(msgg => {
               msg2 = msgg;
               game.players.forEach(playerr => {
                   if(player.displayName != playerr.displayName && playerr.isAlive){
                       msgg.react(playerr.emoteID);
