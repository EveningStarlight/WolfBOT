   ...
   chooseAction: function(guild,game,player) {
       if(player.isAlive){
           if(game.nightNum != 1 && !noBull.includes(player.id)){
               ...
               guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"Select someone to shoot and kill. You have "+numBullets+" bullet(s)."+"```").catch(console.error).then(msgg => {
                   game.players.forEach(playerr => {
                    ...
                    collector.on('collect', (reaction, user) => {
                        ...
                        game.players.forEach(playerr => {
                            if(reaction.emoji.name === playerr.emoteName){
                                ...
                                console.log(player.displayName+" has selected "+playerr.displayName+" as the Vigilante.");
                            }
                    ...
                    collector.on('end', collected => {
                        if(variableName != null){
                            guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected to shoot "+variableName+". You have "+(numBullets-1)+" remaining bullet(s)."+"```").catch(console.error);
                            if(oneBull.includes(player.id)){
               ...
               guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You cannot shoot on the first night."+"```").catch(console.error);
           }
           else if(noBull.includes(player.id)){
               guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You are out of bullets."+"```").catch(console.error);
           }
       }
   }
}
