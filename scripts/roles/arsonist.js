module.exports = {
   chooseAction: function(guild,game,player) {
       if(player.isAlive){
           let mojiArray = [];
           let collector;
           guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"Select a player to douse them in gas, or select the fire to ignite."+"```").catch(console.error).then(msgg => {
                ...
                collector.on('collect', (reaction, user) => {
                    ...
                    game.players.forEach(playerr => {
                        if(reaction.emoji.name === playerr.emoteName){
                            ...
                            console.log(player.displayName+" has selected "+playerr.displayName+" as the Arsonist.");
                        }
                        else if(reaction.emoji.name === '🔥'){
                            variableName = "Ignite";
                            player.action = "Ignite";
                            game.updateActions(user);
                            console.log(player.displayName+" has selected Ignite as the Mass Murderer.");
                        }
                        else if(reaction.emoji.name === '🚫'){
                            variableName = null;
                            player.action = "";
                ...
                collector.on('end', collected => {
                    if(variableName != null){
                        if(variableName == "Ignite"){
                            guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected to ignite everyone doused in gas."+"```").catch(console.error);
                            
                        }
                        else{
                            guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected "+variableName+" to douse in gas."+"```").catch(console.error);
                        }
