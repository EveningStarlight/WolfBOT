           ...
           guild.channels.find(channel => channel.name === "werewolf").send("```css\n"+"Select a player to attack."+"```").catch(console.error).then(msgg => {
               game.players.forEach(playerr => {
                ...
                const filter = (reaction, user) => {
                    return mojiArray.includes(reaction.emoji.name) && user.id != '689650623871975474';
                };
                ...
                collector.on('collect', (reaction, user) => {
                    ...
                    else if (previousPlayer != user.id && previousPlayer != '689650623871975474'){ 
                    ...
                    game.players.forEach(playerr => {
                        if(reaction.emoji.name === playerr.emoteName){
                            ...
                            console.log(game.wolfName+" has selected "+playerr.displayName+" as the Werewolves.");
                        }
