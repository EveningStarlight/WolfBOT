module.exports = {
   chooseAction: function(guild,game,player) {
       if(player.isAlive){
               ...
               game.players.forEach(playerr => {
                        ...
                        if(reaction.emoji.name === playerr.emoteName){
                            ...
                            if(selectedOne == selectedTwo){
                                ...
                                msg2.reactions.forEach(reactionn => {
                                    if(reactionn.emoji.name === reaction.emoji.name){
                                        reactionn.remove(player.id);
                                    }   
                                ...
                                player.action = selectedOne+" and "+selectedTwo;
                                game.updateActions(user);
                                console.log(player.displayName+" has selected "+selectedOne+" and "+selectedTwo+" as the Transporter.");
                ...
                collector.on('end', collected => {
                    if(selectedOne != null && selectedTwo != null){
                        guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"You selected "+selectedOne+" and "+selectedTwo+" to transport."+"```").catch(console.error);
                    }
                    else{
           ...
           guild.channels.find(channel => channel.name === player.displayName.toLowerCase()).send("```css\n"+"Select a second player to swap with the first."+"```").catch(console.error).then(msgg => {
               msg2 = msgg;
               game.players.forEach(playerr => {
                   if(playerr.isAlive){
                       msgg.react(playerr.emoteID);
