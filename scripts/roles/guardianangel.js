module.exports = {
    chooseAction: function (guild, game, player) {
        ...
        guild.channels
            .find(
                (channel) => channel.name === player.displayName.toLowerCase()
            )
            .send(
                '```css\n' +
                    'Select if you would like to protect or not.' +
                    '```'
            )
            .catch(console.error)
            .then((msgg) => {
                ...
                collector.on('collect', (reaction, user) => {
                    ...
                    game.players.forEach((playerr) => {
                        if (reaction.emoji.name === 'heal') {
                            ...
                            console.log(
                                player.displayName +
                                    ' has selected Protect as the Guardian Angel.'
                            )
                ...
                collector.on('end', (collected) => {
                    if (variableName != null) {
                        guild.channels
                            ...
                            .send(
                                '```css\n' +
                                    'You selected to protect your target.' +
                                    '```'
                            )
                            .catch(console.error)
                    } else {
