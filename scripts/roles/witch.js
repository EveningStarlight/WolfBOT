    ...
    chooseAction: function (guild, game, player) {
        if (player.isAlive) {
            ...
            guild.channels
                ...
                .send(
                    '```css\n' +
                        'Select which of your abilities you would like to use.' +
                        '```'
                )
                .catch(console.error)
                .then((msgg) => {
                    msgg.react('722531413802614879')
                        .then(() => msgg.react('722531473546149989'))
                        .then(() => msgg.react('🚫'))
                    const filter = (reaction, user) => {
                        return (
                            ['kill', 'health', '🚫'].includes(
                                reaction.emoji.name
                            ) && user.id === player.id
                        )
                    }
            ...
            guild.channels
                ...
                .send(
                    '```css\n' +
                        'Select a player to use your selected ability on.' +
                        '```'
                )
                .catch(console.error)
                .then((msgg) => {
