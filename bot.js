...
client.on("message", async (msg) => {
  ...
  const isUserHost =
    user.roles.has(discordRoles.Host.id) || msg.author.id === client.user.id; //gives bot the permissions of Host

  if (
    game.channels.undercover != null &&
    game.channels.werewolf != null &&
    channel.name == game.channels.werewolf.name
  ) {
    if (message.substring(0, 1) == "!") {
      guild.channels
        .find((channel) => channel.name === "undercover")
        .send(" !" + message.splice(1));
    } else {
      guild.channels
        .find((channel) => channel.name === "undercover")
        .send(message);
    ...
    switch (cmd) {
      ...
      case "start":
        if (game.isActive && game.isStarted) {
          channel.send(
            user +
              ", there is already an active game. Use *!play* to spectate as a ghost."
          );
          ...
          if (args.length != 0 && playGroups[args[0]] != null) {
            game.playGroup =
              playGroups[args[0]]["players_" + game.players.number()];
            console.log(args[0] + " play group selected");
          } else {
            game.playGroup =
              playGroups["normal"]["players_" + game.players.number()];
      ...
      case "letmein":
        if (
          game.isActive &&
          !game.isStarted &&
          !user.roles.has(discordRoles.Town.id) &&
      ...
      case "j":
        if (
          ...
          if (user.id == "230433217147043840") {
            //user is Buster
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:buster:701416891305951323>"
              )
              .catch(console.error);
          } else if (user.id == "298116458851074070") {
            //user is Eve
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(user.displayName + " has joined the lobby. <3")
              .catch(console.error);
          } else if (user.id == "688911285655699458") {
            //user is Alex
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:alex:701418908963438643>"
              )
              .catch(console.error);
          } else if (user.id == "164246870179315713") {
            //user is Andrew
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:andrew:701416894502010940>"
              )
              .catch(console.error);
          } else if (user.id == "332317117816897536") {
            //user is Andy
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:andy:701416895454380093>"
              )
              .catch(console.error);
          } else if (user.id == "186646350220361728") {
            //user is Ariq
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:ariq:701417165072367668>"
              )
              .catch(console.error);
          } else if (user.id == "382990666873044992") {
            //user is Dayna
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:mild_dayna:701416986801864714>"
              )
              .catch(console.error);
          } else if (user.id == "239257392649338890") {
            //user is Gaelan
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:gaelan:701420026862895204>"
              )
              .catch(console.error);
          } else if (user.id == "324734610997116928") {
            //user is Julianna
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:juliemoji:701416889385222225>"
              )
              .catch(console.error);
          } else if (user.id == "170632428182831104") {
            //user is Morgan
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:morgan:701416892153200650>"
              )
              .catch(console.error);
          } else if (user.id == "135053966353367040") {
            //user is Rafe
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:rafe_wtf:701416986143490079>"
              )
              .catch(console.error);
          } else if (user.id == "130435999259033600") {
            //user is Chandler
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName +
                  " has joined the lobby. <:chandy_boy:701416989796597830>"
              )
              .catch(console.error);
          } else {
            guild.channels
              .find((channel) => channel.name === "join_game")
              .send(
                user.displayName + " has joined the lobby. " + emoji_list[index]
              )
              .catch(console.error);
          }
      ...
      case "day":
        if (!game.isDay && game.isStarted && isUserHost) {
          ...
          for (let channelMember of guild.channels.find(
            (channel) => channel.name === "voice"
          ).members) {
          ...
          game.channels.day.send(
            ":sunny: The sun rises, and you wake for the day."
          );
        }
        break;
      // !night~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      case "night":
        if (game.isDay && game.isStarted && isUserHost) {
          ...
          game.channels.day.send(
            ":crescent_moon: The sun sets, and you go to sleep."
          );
        }
        break;
      case "role":
      ...
      // !nominate~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      case "nominate":
        let accused = await game.players.find(
          (user) => user.id === msg.mentions.users.first().id
        );
        if (accused == null) {
        ...
        if (
          ...
          vote == null &&
          user.roles.has(discordRoles.Town.id) &&
          accused.roles.has(discordRoles.Town.id) &&
          accused.id != user.id
        ) {
          if (nominate.nominations[user.displayName.toLowerCase()] != null) {
            nominate.accused[
              ...
              ].nominated.displayName.toLowerCase()
            ] = null;
          }
          ...
          nominate.nominations[user.displayName.toLowerCase()].nominations = [
            user.displayName,
          ];
          nominate.accused[accused.displayName.toLowerCase()] =
            nominate.nominations[user.displayName.toLowerCase()];

          if (nominate.message != null) {
            nominate.message.delete(1000);
          }
          await updateNominateEmbed();
          nominate.message = await guild.channels
            .find((channel) => channel.name === "day")
            .send(nominate.embed);
        }
        break;
      // !second~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      case "second":
      case "third":
        let accusedS = await game.players.find(
          (user) => user.id === msg.mentions.users.first().id
        );
        if (accusedS == null) {
        ...
        if (
          game.isDay &&
          vote == null &&
          nominate.accused[accusedS.displayName.toLowerCase()] != null &&
          user.roles.has(discordRoles.Town.id) &&
          accusedS.id != user.id
        ) {
          let alreadySecond = false;
          nominate.accused[
            accusedS.displayName.toLowerCase()
          ].nominations.forEach((nomination) => {
            if (nomination == user.displayName) {
              alreadySecond = true;
            }
          });
          if (!alreadySecond) {
            nominate.accused[
              accusedS.displayName.toLowerCase()
            ].nominations.push(user.displayName);
            if (nominate.message != null) {
              nominate.message.delete(1000);
            }
            await updateNominateEmbed();
            ...
            if (
              (nominate.accused[accusedS.displayName.toLowerCase()].nominations
                .length == 2 &&
                numAlive < 9) ||
              (nominate.accused[accusedS.displayName.toLowerCase()].nominations
                .length == 3 &&
      ...
      case "action":
        if (
          ...
          if (channel.name == game.channels.werewolf.name) {
            ...
            msg.reply(
              "Your action has been set to: " +
                game.players.find((player) => player.id === user.id).action
            );
            game.updateActions(user);
          }
        }
      ...
      case "whisper":
        if (
          ...
          channel ==
            guild.channels.find(
              (channel) => channel.name === user.displayName.toLowerCase()
            )
        ) {
          const userMentioned = getChannelFromText(
            args[0].toLowerCase(),
            guild
          );
          ...
          if (!userMentioned || !userMen) {
            ...
            userMen.roles.has(discordRoles.Town.id) &&
            message != "!whisper " + args[0]
          ) {
            userMentioned.send(
              user +
                " whispered: " +
                message.replace("!whisper " + args[0] + " ", "")
            );
            guild.channels
              .find((channel) => channel.name === "day")
              .send(user + " whispered to " + userMen + "!");
        ...
        break;
      case "lynch":
        var lynchedVillager = await game.players.find(
          (user) => user.id === msg.mentions.users.first().id
        );
        if (!isUserHost) {
      ...
      case "innocent":
        if (
          user.roles.has(discordRoles.Town.id) &&
          vote != null &&
          vote.players.remaining.indexOf(user) != -1
        ) {
          vote.players.innocent.push(
            vote.players.remaining.splice(
              vote.players.remaining.indexOf(user),
              1
            )
          );
          vote.innocent++;
          ...
          game.channels.day.send(
            "The Town has decided  " + vote.player + " is innocent"
          );
          vote = null;
          for (let channelMember of guild.channels.find(
            (channel) => channel.name === "voice"
          ...
          guild.channels
            .find((channel) => channel.name === "day")
            .overwritePermissions(discordRoles.Town, { SEND_MESSAGES: true });
        } else {
          msg.reply(
            "Oak's words echoed... There's a time and place for everything, but not now."
          );
      ...
      case "abstain":
        if (
          ...
          game.channels.day.send(
            "The Town has decided that " + vote.player + " is guilty"
          );
          game.channels.day.send("!kill " + vote.player);
          vote = null;
          for (let channelMember of guild.channels.find(
      ...
      // !kill~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      case "kill":
        //Command must be formatted as !kill @Eve
        if (isUserHost) {
          numAlive = numAlive - 1;
          var killedVillager = await game.players.find(
            (user) => user.id === msg.mentions.users.first().id
          );

          if (killedVillager.roles.has(discordRoles.Town.id)) {
            channel.send(killedVillager.toString() + " has been killed");
            await killedVillager
              .removeRole(discordRoles.Town)
              .catch(console.error);
            await killedVillager
              .addRole(discordRoles.Dead)
              .catch(console.error);
      ...
      case "help":
        ...
        if (isUserHost) {
          embed
            ...
            .addField(
              "!day",
              "Set's the time to day and allows people to speak in the day channels."
            )
            .addField(
              "!night",
              "Set's the time to night and mutes all players in the voice channel and open the night text channels."
            )
            .addField(
              "!lynch @name",
              "Put's that player up on the stand and gives them 30 seconds to make their case."
            )
            .addField(
              "!skip",
              "To skip the 30 seconds above and move directly to the 30 seconds of group discussion preiod."
            )
            .addField(
              "!clear",
              "To end an open game, removing all roles, and moving players back to the General voice channel."
            )
          ...
          embed
            ...
            .addField(
              "!role rolename",
              "Gives a description of the named role."
            )
            .addField("!action text", "Sends the passed text to the host")
            .addField(
              "!whisper playername msg",
              "To whisper a message to that player."
            )
            .addField("!nominate @name", "Nominate a player named.")
            .addField(
              "!second @name",
              "Second someone that someone else nominated."
            )
          ...
          embed
            ...
            .addField("!start", "To open a lobby, or to start a full lobby.")
            .addField(
              "!play (join, letmein, p, j)",
              "Joins an open lobby, or spectates an open game."
            )
            .addField(
              "!role rolename",
              "Gives a description of the named role."
      ...
      default:
        msg.reply(
          cmd + " is not a valid command. Use !help for more information!"
        );
        break;
    }
    msg.delete(1000);
...
async function invitePlayers(guild, host) {
  ...
  await updateJoinEmbed();
  joinMessage = await guild.channels
    .find((channel) => channel.name === "join_game")
    .send(embedMSG);
}
...
function startGame(guild) {
  ...
  createChannel(guild, discordChannels.voice).then((channel) => {
    ...
    let generalVoice = guild.channels.find(
...
function newGame() {
  ...
  game.updateActions = async function (user) {
    ...
    game.players.forEach((player) => {
      embed.addField(
        player.displayName + " - " + player.role.roleName,
...
async function endGame(guild) {
  ...
  let gameVoice = await guild.channels.find(
    (channel) => channel.name === "voice"
  );
  if (gameVoice != null) {
  ...
  Object.values(discordChannels).forEach(async (obj) => {
    let channel = await guild.channels.find(
      (channel) => channel.name === obj.name
    );
    if (channel != null) {
...
function createUserChannels(user, guild) {
  ...
  channelObj.message =
    "This is your personal channel for communicating with the host, and for using commands";
  ...
  createChannel(guild, channelObj)
    .then(() => {
      let channel = game.channels[user.displayName];
      channel.overwritePermissions(
        user,
        discordChannels.template.rolePermission
      );
...
async function printPlayGroup(channel, playGroup) {
  ...
  playGroup.forEach((role) => {
    string += role.Main;
    if (role.Sub != null && role.Sub != "Werewolf") {
      string += " " + role.Sub;
    }
    string += "\n";
  });
    ...
    for (channelName of rolesIG[numRoles].channels) {
      if (game.channels[channelName] == null) {
        await createChannel(guild, discordChannels[channelName]).then(() => {
          game.channels[channelName].overwritePermissions(
            user,
            discordChannels[channelName].rolePermission
          );
        });
      } else {
        game.channels[channelName].overwritePermissions(
...
async function updateNominateEmbed() {
  ...
  Object.values(nominate.nominations).forEach((nomination) => {
    ...
    if (nomination.nominations.length > 2) {
      string += "\nThird: " + nomination.nominations[2];
    }

    nominate.embed.addField(
      "Nominated: " + nomination.nominated.displayName,
      string
    );
  });
}
...
async function beginLynch(guild, lynchedVillager) {
  ...
  guild.channels
    .find((channel) => channel.name === "day")
    .send(
      vote.player.toString() +
        " has been put up on the stand! " +
        vote.player.toString() +
        " has 30 seconds to make their case!"
    );
  ...
  for (let channelMember of guild.channels.find(
    ...
    if (
      channelMember[1].roles.has(discordRoles.Town.id) &&
      channelMember[1] != vote.player
    ) {
      channelMember[1].setMute(true);
    }
  }
  ...
  await Promise.all(arrayPromises).catch((err) => console.log(err));
  await updateVoteEmbed();
  vote.message = await guild.channels
    .find((channel) => channel.name === "host")
    .send(vote.embed);
...
function unMuteTown(guild) {
  guild.channels
    .find((channel) => channel.name === "day")
    .send("The town has 30sec to discuss what they heard!");
  for (let channelMember of guild.channels.find(
    (channel) => channel.name === "voice"
  ).members) {
    if (channelMember[1].roles.has(discordRoles.Town.id)) {
...
function muteAllVote(guild) {
  guild.channels
    .find((channel) => channel.name === "day")
    .send("Everyone must now vote! Either !guilty | !innocent | !abstain");
  for (let channelMember of guild.channels.find(
    (channel) => channel.name === "voice"
  ).members) {
    if (channelMember[1].roles.has(discordRoles.Town.id)) {
  ...
  guild.channels
    .find((channel) => channel.name === "day")
    .overwritePermissions(discordRoles.Town, { SEND_MESSAGES: false });
  voting = true;
}
...
async function moveVoiceChannels(oldChannel, newChannel) {
  console.log(
    "Moving all players from " + oldChannel.name + " to " + newChannel.name
  );
  const movePromises = [];
...
function createChannel(guild, channelObj) {
  return guild
    .createChannel(channelObj.name, channelObj.type)
    .then((channel) => {
      ...
      let permissions = discordChannels.template.permissions.concat(
        channelObj.permissions
      );
